/* Turning settings into CSS.
   ...
   Every visual preference — palette, wood, light, type — is expressed as a
   ...
   The approach is calendartospare's; the token set is this app's own. */

/* The interface is near-neutral, so there are only four colours that shape
   it — and buttons follow the accent rather than having a colour of their own,
   which is why there is no separate button entry here. The six spine colours
   below are for books and tags, where colour is doing a job. */
export const PALETTE_KEYS = [
  { id: 'paper', label: 'Paper', hint: 'the page you write on' },
  { id: 'ink', label: 'Ink', hint: 'body text' },
  { id: 'accent', label: 'Accent', hint: 'headings, links and buttons' },
  { id: 'muted', label: 'Muted', hint: 'page numbers, captions' },
];

export const SPINE_KEYS = [
  { id: 'tag-1', label: 'Spine 1' },
  { id: 'tag-2', label: 'Spine 2' },
  { id: 'tag-3', label: 'Spine 3' },
  { id: 'tag-4', label: 'Spine 4' },
  { id: 'tag-5', label: 'Spine 5' },
  { id: 'tag-6', label: 'Spine 6' },
];

const ALL_KEYS = [...PALETTE_KEYS, ...SPINE_KEYS];
...
/** The next light state when the cord is pulled or the lamp is clicked. */
export function toggleLight(settings) {
  const night =
    settings.theme === 'dark' ||
    (settings.theme === 'system' &&
      window.matchMedia('(prefers-color-scheme: dark)').matches);

  // By day the cord closes the blinds; by night the switch swaps moonlight
  // for a bulb. Two different gestures, one piece of state.
  if (night) return settings.light === 'bulb' ? 'on' : 'bulb';
  return settings.light === 'off' ? 'on' : 'off';
}
...
/** How a closed cookbook is bound. */
export const COVER_STYLES = [
  { id: 'plain', label: 'Plain' },
  { id: 'halfbound', label: 'Half-bound' },
  { id: 'banded', label: 'Banded' },
  { id: 'label', label: 'Labelled' },
  { id: 'cloth', label: 'Cloth' },
];

/** The paper a cookbook is written on. */
